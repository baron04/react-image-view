...
界面部件可以通过组合自由增删；手势行为则集中由 `Stage` 管理，当前不能靠少渲染
一个按钮单独关闭。这个边界是刻意的：视觉组合保持开放，指针状态机仍只有一份。
...
界面文案基本都只以 `aria-label` 的形式存在——默认 UI 渲染的是图标，唯一的可见文字是错误标题。应用需要让这些名称与页面声明的语言保持一致，否则读屏软件仍会照字面念出错误语言的标签。

库不根据 `navigator.language` 猜测内容语言；它默认使用稳定的英文文案，语言由应用显式决定。简体中文可以从
