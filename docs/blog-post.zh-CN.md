...
> **先说结论**：[react-img-view](https://github.com/baron04/react-img-view) 是一个 headless-first、可组合的 React 图片预览组件。缩放、拖拽、双指捏合、旋转、1:1、键盘快捷键、触摸手势全部内置；默认界面开箱即用，也可以把每一个可见部件拆下来换成你自己的。轻量、零运行时依赖，MIT 协议。
