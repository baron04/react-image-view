...
| `onKeyDown`              | 见下                      | —                     | 在内置快捷键之前运行，返回 `true` 表示这个键已被接管。                          |
...
内置两种语言：`en` 由主入口和 primitives 入口导出，也是 `Group` 的回退值；
简体中文放在独立子路径按需引入，不会增加默认包体积：
...
其它语言直接传一个对象即可，没有语言包注册表要登记，也没有东西要引入 ——
`labels` 会按字段合并到 `en` 之上，所以只传一部分也可以，没覆盖的键继续用英文：
...
完整键名清单见上面的 [`ViewerLabels`](#viewerlabels)。除 `errorTitle` 外全都是
`aria-label`，漏翻不会显示成错字，而是变成无障碍缺陷。

## 其它入口

### `react-img-view/primitives`

同名的同一批部件，但不带预设。`Group` 和 `ImageView` 都是主入口对应组件的
...
| `container` / `onKeyDown` / `labels` | 与 `Group` 相同                            | —       | 转发给内部托管的 `Group`。                      |
