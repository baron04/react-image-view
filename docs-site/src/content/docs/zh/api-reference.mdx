...
从 `react-img-view/imperative` 引入 `ImagePreview`。这个独立入口不会增加
常规入口的体积，也不需要 Provider：
...
关闭按钮还是 Escape 关闭，React Root 和临时宿主都会自动卸载。
...
最终 DOM 必须包含一个展开了 `imageProps` 的 `<img>`。这些属性包含图片地址、
尺寸样式、加载/错误处理，以及变换和重试所需的连接。想保留预设界面时，可把同一个
