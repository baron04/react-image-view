...
这里把常见需求按改动范围列出来：改样式、替换按钮或大图、增加自己的控件，
...
<Aside type="caution" title="asChild 的子组件必须转发 props 和 ref">
  控件是通过 Slot 克隆子元素来传递点击处理函数、测量动画用的 ref 和 `data-*`
  的。如果你的子组件把不认识的 props 丢掉了，这些东西会被一起吞掉 ——
...
如果希望自定义界面的包尽可能小，使用下面的 headless 入口；其中的 `Root`
从不引入或补上默认 preset。
...
<Aside title="只想保留大部分默认界面">
  需要 preset 时，从主入口 `react-img-view` 引入 `ImageView`。如果只是想开关内置选项， 显式渲染
  `DefaultContent`，并传入 `counter` 和 `thumbnails` 即可。
</Aside>
...
所需的 `imageProps` 交给你；最终渲染的 `<img>` 必须接收这些属性。
...
对于组合方式够不着的键盘行为，传一个 extension。返回 `true` 表示事件已被消费，
内置处理会被跳过。
