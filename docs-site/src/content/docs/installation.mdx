...
See [Server rendering and Next.js](#server-rendering-and-nextjs) below.
...
Labels are the one thing to set deliberately. `Root` falls back to fixed
...
`asChild`, while `Trigger` always reuses its only child. That is enough to
